...
class TestURLTree(unittest.TestCase):
    ...
    def test_root(self):
        ...
        expected = {URLNode('http://w4af.org', 1): {}}
        self.assertEqual(tree.tree, expected)

    def test_two_independent_paths(self):
        tree = URLTree()

        url_1 = URL('http://w4af.org/foo/')
        url_2 = URL('http://w4af.org/bar/')
        tree.add_url(url_1)
        tree.add_url(url_2)

        expected = {URLNode("http://w4af.org", 0): {URLNode("foo", 1): {},
                                                    URLNode("bar", 1): {}}}
        self.assertEqual(tree.tree, expected)
    ...
    def test_two_nested_paths(self):
        ...
        url_1 = URL('http://w4af.org/foo/bar/')
        url_2 = URL('http://w4af.org/spam/eggs/')
        tree.add_url(url_1)
        tree.add_url(url_2)

        expected = {URLNode("http://w4af.org", 0):
    ...
    def test_nested_paths_and_files(self):
        ...
        url_1 = URL('http://w4af.org/foo/bar/')
        url_2 = URL('http://w4af.org/spam/eggs/123.txt')
        tree.add_url(url_1)
        tree.add_url(url_2)
