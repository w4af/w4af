...
class finger_google(InfrastructurePlugin):
    """
    Search Bing using the Bing API to get a list of users for a domain.
    ...
    def discover(self, fuzzable_request, debugging_id):
        ...
        self._bing = bing(self._uri_opener)
    ...
    def _do_fast_search(self):
        """
        Only search for mail addresses in the bing result page.
        """
        search_string = '@' + self._domain_root
        result_page_objects = self._bing.get_n_result_pages(search_string, self._result_limit)
    ...
    def _do_complete_search(self):
        ...
        """
        search_string = '@' + self._domain_root
        bing_results = self._bing.search(search_string, self._result_limit)
        self.worker_pool.map(self._find_accounts, bing_results)

    def _find_accounts(self, bing_result):
        """
        Finds emails in Bing result page.

        :param bing_result: BingResult instance
        :return: A list of valid accounts
        """
        om.out.debug('Searching for emails in: ' + bing_result.URL)

        grep_res = bing_result.URL.get_domain() == self._domain

        response = self._uri_opener.GET(bing_result.URL,
                                        cache=True,
                                        grep=grep_res)
    ...
    def get_options(self):
        ...
        h = ('This method is faster, because it only searches for emails in'
             ' the small page snippet that Bing shows to the user after'
    ...
    def get_long_desc(self):
        ...
        """
        return """
        This plugin finds mail addresses in Bing.

        Two configurable parameters exist:
            - result_limit
            - fast_search

        If fast_search is set to False, this plugin searches bing for:
        ...
        set to True, only mail addresses that appear on the bing result page
