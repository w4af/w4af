...
from w4af.core.data.constants.severity import HIGH
from w4af.core.data.options.opt_factory import opt_factory
from w4af.core.data.options.option_types import BOOL, STRING
from w4af.core.data.options.option_list import OptionList
from w4af.plugins.output.console import catch_ioerror
...
class system_log(OutputPlugin):
    ...
    #
    # We'll match those to the levels we have in w4af.
    #
    ...
    def set_options(self, option_list):
        ...
        syslog.openlog('w4af', logoption=syslog.LOG_PID)
    ...
    def get_long_desc(self):
        ...
        Log lines generated by this plugin will have the following format:
            ...
            {date} {hostname} w4af[{PID}]: [{scan-id}] {message}
        
        Where the fields are defined as:
        
            - {date}: The log entry timestamp
            - {hostname}: The operating system host name
            - {PID}: The w4af process ID
            - {scan-id}: The configured scan identifier
            - {message}: Log message sent by w4af
        
        An example log line is:
        
            Sep  8 16:33:12 workstation w4af[2888]: [2k0jzvbwuk] Hello world!
