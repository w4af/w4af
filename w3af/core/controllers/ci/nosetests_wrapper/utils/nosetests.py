...
def get_nosetests_filename(suffix, first, last):
...
def run_nosetests(nose_cmd, first, last):
    ...
    try:
        ...
        with open(get_nosetests_filename('log', first, last)) as output_file:
            with open(get_nosetests_filename(NOSE_XUNIT_EXT, first, last)) as xunit_output:
