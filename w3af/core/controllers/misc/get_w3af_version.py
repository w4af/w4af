...
from w4af.core.controllers.auto_update.utils import (is_git_repo, to_short_id,
                                                     get_latest_commit,
...
@memoized
def get_w4af_version_as_dict():
    """
    This method seems to take considerable time to run when w4af is run from
    a git installation (.git directory is present). All of the time it takes
    to solve this function comes from get_w4af_version_as_dict(), which
    reads the Git meta-data.

    Some plugins, such as xml_file, call get_w4af_version every N seconds to
...
def get_w4af_version():
    """
    :return: A string with the w4af version.
    """
