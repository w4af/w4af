...
from w4af.core.data.fuzzer.mutants.urlparts_mutant import URLPartsContainer
...
class TestFileNameMutant(unittest.TestCase):
    ...
    def test_basics(self):
        ...
        freq = FuzzableRequest(URL('http://www.w4af.com/foo/bar.htm'))
        m = FileNameMutant(freq)
        m.set_dc(parts)

        self.assertEqual(m.get_url().url_string,
                         'http://www.w4af.com/foo/ping%21.htm')

        expected_found_at = '"http://www.w4af.com/foo/ping%21.htm", using HTTP'\
    ...
    def test_valid_results(self):
        ...
        expected_urls = [URL('http://www.w4af.com/foo/abc.htm'),
                         URL('http://www.w4af.com/foo/def.htm'),
                         URL('http://www.w4af.com/foo/bar.abc'),
                         URL('http://www.w4af.com/foo/bar.def')]
    ...
    def test_valid_results_double_encoding(self):
        ...
        freq = FuzzableRequest(URL('http://www.w4af.com/bar.htm'))

        generated_mutants = FileNameMutant.create_mutants(freq, payloads, [],
                                                          False,
                                                          self.fuzzer_config)

        expected_urls = ['http://www.w4af.com/ls+-+la.htm',
                         'http://www.w4af.com/bar.ls+-+la',
                         'http://www.w4af.com/http%3A%2F%2F127.0.0.1%3A8015%2Ftest%2F.htm',
                         'http://www.w4af.com/http%3A//127.0.0.1%3A8015/test/.htm',
                         'http://www.w4af.com/bar.http%3A%2F%2F127.0.0.1%3A8015%2Ftest%2F',
                         'http://www.w4af.com/bar.http%3A//127.0.0.1%3A8015/test/']
