...
from w4af.core.data.request.request_mixin import RequestMixIn
from w4af.core.data.constants.encodings import DEFAULT_ENCODING
from w4af.core.data.misc.encoding import smart_str, smart_str_ignore, smart_unicode
...
class FuzzableRequest(RequestMixIn, DiskItem):
    ...
    to allow w4af plugins to be much simpler and don't really care if the
    ...
    def from_base64(cls, base64_data):
        ...
        """
        from w4af.core.data.parsers.doc.http_request_parser import raw_http_request_parser
    ...
    def __hash__(self):
        ...
        if data is None:
            data = "<EMPTY>"
        return hash(str(self.get_uri()) + data)
