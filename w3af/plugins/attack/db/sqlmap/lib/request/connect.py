...
class Connect(object):
    ...
    def _connReadProxy(conn):
        ...
        if not kb.dnsMode and conn:
            headers = conn.info()
            if headers and hasattr(headers, "get") and (headers.get(HTTP_HEADER.CONTENT_ENCODING, "").lower() in ("gzip", "deflate")\
              or "text" not in headers.get(HTTP_HEADER.CONTENT_TYPE, "").lower()):
